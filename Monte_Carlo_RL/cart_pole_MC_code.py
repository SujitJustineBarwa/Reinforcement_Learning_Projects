...
class action_value_function:
    ...
    def save_q_table(self, filename="MC_q_table.csv"):
        """Save Q-table to a CSV file"""
        df = pd.DataFrame(self.q_table)
        df.to_csv(filename, index=False)
        print(f"Q-table saved to {filename}")

    def load_q_table(self, filename="MC_q_table.csv"):
...
alpha = 0.1
eps = 0.1
if os.path.exists("MC_q_table.csv"):
    q_func.load_q_table()
...
for ep_num in range(1000):
    ...
    while not episode_over:
        ...
        records.append((t, current_state, action, reward))
        t += 1
        ...
        if terminated or truncated:
            ...
            G = 0
            for (t_step, state, action, reward) in reversed(records):
                G = reward + discount_factor * G

                # Update Q-value using the reward and the estimated return
                q_value = q_func.get_q_value(state, action)
                updated_q_value = q_value + alpha * (G - q_value)
                q_func.set_q_value(state, action, updated_q_value)
